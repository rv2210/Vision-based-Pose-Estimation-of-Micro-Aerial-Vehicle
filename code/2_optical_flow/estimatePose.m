...
% should made from this function.
    ...
    h_matrix=transpose(reshape(h,[3,3]));

    singular_values= svd(h_matrix);

    h_normalized= h_matrix/ singular_values(2);

    sign_h= transpose([data(t).p0(1:2,1);1]) * h_normalized * [res(1:2,1);1];
    ...
    R_c_w= u2 * [ 1 0 0 ; 0 1 0 ; 0 0 det(u2*v2_transpose)]* v2_transpose ; 
    t_c_w= t_cap/norm(r1_cap);
